...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (page == 3) {
        ...
        self.tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(toMainView)];
...
- (void)viewDidLoad
{
    ...
    UIButton *sender = [[UIButton alloc]initWithFrame:CGRectMake(kScreenWidth-60, 20, 50, 35)];
    [sender setTitle:@"跳过" forState:UIControlStateNormal];
    [sender setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    sender.layer.borderColor = [UIColor brownColor].CGColor;
    sender.layer.borderWidth = 1;
    sender.layer.cornerRadius = 4;
    [sender addTarget:self action:@selector(toMainView) forControlEvents:UIControlEventTouchUpInside];
